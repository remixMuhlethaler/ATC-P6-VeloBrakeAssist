...
**An open-source adaptive cycling device for users with limb differences.**
...
- **Braking system** — a stainless steel cable mechanism that transmits the user's wrist/arm movement into modulated brake actuation.
- **Attachment mechanism** — a quick-mount PETCF bracket that clamps to the handlebar without tools.
...
![VeloBrake Assist Validation Footage](media/Concept/ATC_breaking_part.mp4) 
---
...
| Component | Material / Ref | Source | Approx. cost |
|---|---|---|---|
| Hand-grip body | TPU (flexible) | EPFL 3D print lab | — |
| Structural shell | PETG + PET-CF | EPFL 3D print lab | — |
| Attachment bracket | PET-CF | EPFL 3D print lab | — |
| Stainless steel cable | Standard brake cable | Local bike shop | ~25 CHF |
| Cable crimp ends | Standard | OBI / EPFL SPOT | — |
| Screws & heat inserts | M3/M4 assorted | OBI / EPFL SPOT | ~96 CHF |
| Sleeve padding | Neoprene sleeve | Online | ~95 CHF |

**Total build cost: ~343 CHF** (initial budget: 1000 CHF)
...
3. **Attach to the handlebar** — slide the bracket over the handlebar and tighten the clamp screw. No tools required for daily use.
...
- Each subsystem went through design, prototyping, verification (bench tests), and validation (on-bike tests with the user).
